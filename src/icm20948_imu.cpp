...
// Static field
SPISettings ICM20948_IMU::spiSettings = SPISettings(7000000, MSBFIRST, SPI_MODE2);

// Constructors
ICM20948_IMU::ICM20948_IMU(mbed::SPI* spiBus, uint8_t cs_pin, xyzOffset accel_offset, xyzOffset gyro_offset) : accel_offset(accel_offset), gyro_offset(gyro_offset) {
    this -> spiBus = spiBus;
    this -> CS_PIN = cs_pin;
    currentBank = 0;
...
// Public methods
SPISettings ICM20948_IMU::getSpiSettings() {
    return ICM20948_IMU::spiSettings;
}
...
uint8_t ICM20948_IMU::readFifo() {
    uint8_t val = readRegister8(0, ICM20948_FIFO_R_W);
    return val;
}
...
xyzFloat ICM20948_IMU::accelData() {
...
xyzFloat ICM20948_IMU::gyroData() {
...
int16_t ICM20948_IMU::readRegister16(uint8_t bank, uint8_t reg) {
    ...
    digitalWrite(CS_PIN, LOW);
    spiBus->transfer(reg);
    result = (int16_t) spiBus->transfer16(0x00);
    digitalWrite(CS_PIN, HIGH);
...
void ICM20948_IMU::switchBank(uint8_t bank) {
    if (bank != currentBank) {
        currentBank = bank;
        digitalWrite(CS_PIN, LOW);
        spiBus->transfer(ICM20948_REG_BANK_SEL);
        spiBus->transfer(bank);
        digitalWrite(CS_PIN, HIGH);
    }
}

void ICM20948_IMU::startFifo() {
    // writeRegister8(0, ICM20948_FIFO_EN_2);
}
